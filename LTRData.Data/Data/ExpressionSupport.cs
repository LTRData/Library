...
public static class ExpressionSupport
{
    ...
    private static Dictionary<string, MethodInfo?> GetCompatibleMethod_methodCache => field ??= [];
    ...
    public static MethodInfo? GetCompatibleMethod(Type TypeToSearch, bool FindStaticMethod, string MethodName, Type[] GenericArguments, Type ReturnType, Type[] AlternateArgsTypes)
    {
        ...
        lock (GetCompatibleMethod_methodCache)
        {
            if (!GetCompatibleMethod_methodCache.TryGetValue(key, out newMethod))
            {
                ...
                GetCompatibleMethod_methodCache.Add(key, newMethod);
            }
        }
    ...
    private static Dictionary<Type, Type[]> listItemsTypes => field ??= [];
    ...
    public static Dictionary<IEnumerable<MemberInfo>, string> GetDataFieldMappings(Type ElementType)
    {
        ...
        lock (dataFieldMappings)
        {
            if (!dataFieldMappings.TryGetValue(ElementType, out mappings))
            {
                ...
                foreach (var mapping in from prop in ElementType.GetMembers(BindingFlags.Public | BindingFlags.Instance | BindingFlags.FlattenHierarchy)
                ...
                foreach (var submapping in from props in mappings.Keys.ToArray()
    ...
    private static Dictionary<Type, ReadOnlyCollection<string>> GetPropertiesWithAttributes_cache => field ??= [];
    ...
    private class AttributedMemberFinder<TAttribute> where TAttribute : Attribute
    {
        public static ReadOnlyCollection<string> GetPropertiesWithAttributes(Type type)
        {
            ...
            lock (GetPropertiesWithAttributes_cache)
            {

                if (!GetPropertiesWithAttributes_cache.TryGetValue(type, out prop))
                {
                    ...
                    GetPropertiesWithAttributes_cache.Add(type, prop);
                }
