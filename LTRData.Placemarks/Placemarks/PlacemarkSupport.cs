...
public static class PlacemarkSupport
{
    ...
    public static Region[] ParseKmlRegions(string kmlFile) => ParseKmlRegions(XmlSupport.XmlDeserialize<Kml.Simplified.Kml>(kmlFile)
        ?? throw new ArgumentException($"Invalid XML data in file '{kmlFile}'"));

    public static Region[] ParseKmlRegions(this Kml.Simplified.Kml kmlData) => ParseKmlRegions(kmlData.Document.Folders?[0].Placemarks
        ?? throw new ArgumentException("Invalid kml document"));

    public static async Task<Region[]> ParseKmlRegions(this Task<Kml.Simplified.Kml> kmlData) => ParseKmlRegions((await kmlData.ConfigureAwait(false)).Document.Folders?[0].Placemarks
        ?? throw new ArgumentException("Invalid kml document"));
    ...
    public static Task<Region[]> ParseKmlRegionsAsync(this Kml.Simplified.Kml kmlData) => Task.Run(() => ParseKmlRegions(kmlData));

    public static Task<Region[]> ParseKmlRegionsAsync(this Task<Kml.Simplified.Kml> kmlDataTask) => kmlDataTask.ContinueWith(kmlData => ParseKmlRegions(kmlData.Result));
