...
public static class Math3DExtensions
{
    ...
    public static ConsoleColor GetNearestConsoleColorRGB(this Color Color) => StringFormatting.ParseEnumName<ConsoleColor>(GetClosestConsoleCompatibleColor(Color).GetNearestColorRGB(ConsoleColors).Name);

    public static ConsoleColor GetNearestConsoleColorHSB(this Color Color) => StringFormatting.ParseEnumName<ConsoleColor>(Color.GetNearestColorHSB(ConsoleColors).Name);

#if NETCOREAPP

    private static readonly Color[] ConsoleColors = Array.ConvertAll(Enum.GetNames<ConsoleColor>(), Color.FromName);

    private static readonly Color[] KnownColors = Array.ConvertAll(Enum.GetValues<KnownColor>(), Color.FromKnownColor);
